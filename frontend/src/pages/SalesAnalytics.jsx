...
const MOCK_DATA = {
  ...
  model_distribution: [
    { model: 'KF5700B II', count: 420 },
    { model: 'HD2600SY', count: 310 },
    { model: 'SE2600Y', count: 250 },
    { model: 'L3000SY', count: 180 },
    { model: 'XM3100ST', count: 150 },
    { model: 'F400', count: 120 },
    { model: 'HS5000M', count: 105 },
    { model: 'XF6300', count: 90 },
    { model: 'KL2800', count: 75 },
    { model: 'VMC450', count: 60 },
    { model: 'HD2200', count: 55 },
    { model: 'F500', count: 48 },
    { model: 'KF5600', count: 42 },
    { model: 'HS4000', count: 35 },
    { model: 'SE2000', count: 28 },
  ],
  dealer_share: [
    { name: 'AK MAKINA', value: 320 },
    { name: 'ARO-TEC', value: 280 },
    { name: 'BREMBO', value: 240 },
    { name: 'NEWEMAG', value: 210 },
    { name: 'VIMACCHINE', value: 180 },
    { name: 'WECO', value: 150 },
    { name: 'REPMO', value: 120 },
    { name: 'GMP', value: 100 },
    { name: 'PROFIKA', value: 80 },
    { name: 'NAGEL', value: 60 },
    ...
    <div className="page-body">
      ...
      <div className="kpi-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
        ...
        <div className="glass-card kpi-card">
          ...
          <div className="kpi-value">{formatNumber(MOCK_DATA.kpi.total_orders)} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>대</span></div>
          <div className="kpi-desc">전체 유통 및 발주된 장비 총량</div>
        </div>
        ...
        <div className="glass-card kpi-card">
          ...
          <div className="kpi-value" style={{ color: 'var(--status-production)' }}>{formatNumber(MOCK_DATA.kpi.in_production)} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>대</span></div>
          <div className="kpi-desc">한국 공장에서 제조/조립 중</div>
        </div>
        ...
        <div className="glass-card kpi-card">
          ...
          <div className="kpi-value" style={{ color: 'var(--status-ready)' }}>{formatNumber(MOCK_DATA.kpi.in_shipping)} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>대</span></div>
          <div className="kpi-desc">현재 해상/항공을 통해 이동 중</div>
        </div>
        ...
        <div className="glass-card kpi-card" style={{ paddingBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>함부르크:</span>
              <span style={{ fontWeight: 600 }}>{MOCK_DATA.kpi.atp.hamburg}대</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>네덜란드:</span>
              <span style={{ fontWeight: 600 }}>{MOCK_DATA.kpi.atp.netherlands}대</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>터키:</span>
              <span style={{ fontWeight: 600 }}>{MOCK_DATA.kpi.atp.turkey}대</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(480px, 1fr))', gap: '24px', marginBottom: '24px' }}>
        ...
        <div className="glass-card" style={{ padding: '24px' }}>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart layout="vertical" data={MOCK_DATA.model_distribution} margin={{ top: 5, right: 50, left: 20, bottom: 5 }}>
              ...
              <Bar dataKey="count" radius={[0, 8, 8, 0]} barSize={10} name="판매 대수">
                {MOCK_DATA.model_distribution.map((entry, index) => (
          ...
          <div style={{ display: 'flex', height: '280px', gap: '16px' }}>
            <div style={{ flex: '1', minWidth: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={MOCK_DATA.dealer_share}
                    cx="50%" cy="50%"
                    innerRadius={50} outerRadius={80}
                    dataKey="value"
                    stroke="none"
                  >
                    {MOCK_DATA.dealer_share.map((entry, index) => (
            ...
            <div style={{ flex: '1.5', minWidth: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart layout="vertical" data={MOCK_DATA.dealer_share} margin={{ top: 5, right: 10, left: 50, bottom: 5 }}>
                  <XAxis type="number" hide />
      ...
      <div className="glass-card" style={{ padding: '24px' }}>
        ...
        <div style={{ background: 'var(--bg-secondary)', padding: '24px', borderRadius: '12px', border: '1px solid var(--border-color)', marginBottom: '24px' }}>
          <h4 style={{ fontSize: '1.1rem', marginBottom: '20px' }}>KPI : 7월 (현재 월) 실적 및 목표 달성률 상세</h4>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '20px', marginBottom: '24px' }}>
            <div style={{ background: 'var(--bg-card)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border-color)', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '1.4rem', fontWeight: 'bold', marginTop: '4px' }}>7월</div>
            </div>
            <div style={{ background: 'var(--bg-card)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border-color)', textAlign: 'center' }}>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>현재 실적 (수주액)</div>
              <div style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--wia-blue-vibrant)', marginTop: '4px' }}>€2,000,000</div>
            </div>
            <div style={{ background: 'var(--bg-card)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border-color)', textAlign: 'center' }}>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>수주 목표 금액</div>
              <div style={{ fontSize: '1.2rem', fontWeight: 'bold', marginTop: '4px' }}>€2,000,000</div>
            </div>
            <div style={{ background: 'var(--bg-card)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border-color)', textAlign: 'center' }}>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>현재 수주 대수</div>
              <div style={{ fontSize: '1.4rem', fontWeight: 'bold', color: 'var(--wia-blue-vibrant)', marginTop: '4px' }}>31대</div>
            </div>
            <div style={{ background: 'var(--bg-card)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border-color)', textAlign: 'center' }}>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>수주 목표 수량</div>
              <div style={{ fontSize: '1.4rem', fontWeight: 'bold', marginTop: '4px' }}>30대</div>
            </div>
          </div>
          ...
          <div style={{ background: 'var(--bg-card)', padding: '20px', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
            <div style={{ marginBottom: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                ...
                <span style={{ fontWeight: 700, color: 'var(--wia-blue-vibrant)' }}>100.0%</span>
              </div>
              <div style={{ width: '100%', height: '14px', background: 'var(--bg-secondary)', borderRadius: '7px', overflow: 'hidden', border: '1px solid var(--border-color)' }}>
                <div style={{ width: '100%', height: '100%', background: 'linear-gradient(90deg, var(--wia-blue), var(--wia-blue-light))' }} />
              </div>
            </div>
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <span style={{ fontWeight: 600, fontSize: '0.95rem' }}>수주대수 달성률 (진행률)</span>
                <span style={{ fontWeight: 700, color: 'var(--status-stock)' }}>103.3%</span>
              </div>
              <div style={{ width: '100%', height: '14px', background: 'var(--bg-secondary)', borderRadius: '7px', overflow: 'hidden', border: '1px solid var(--border-color)' }}>
